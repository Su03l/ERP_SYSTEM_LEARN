    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        ...
        <div class="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-brand-100 overflow-hidden animate-fade-in">
            ...
            <div class="p-6" x-data="performanceForm()">
                ...
                <form x-show="employee" x-transition x-cloak method="POST" action="{{ route('performance.store') }}">
                                            ...
                                            @foreach($category['items'] as $itemKey => $itemLabel)
                                                <tr class="hover:bg-brand-50/30 transition">
                                                    ...
                                                    <td class="px-5 py-3 text-center">
                                                        <span class="text-sm font-bold text-brand-400">5</span>
                                                    </td>
                                                    <td class="px-5 py-3 text-center">
                    ...
                    {{-- زر الحفظ --}}
                    <div class="flex justify-end pt-4 mt-4 border-t border-brand-100">
                        <button type="submit"
        ...
        <div class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-brand-900 to-brand-950 rounded-2xl shadow-sm p-6 text-white animate-fade-in">
                ...
                <h4 class="font-black text-lg mb-2">مقياس التقييم</h4>
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between"><span class="text-green-300">ممتاز</span><span class="text-brand-200">90% - 100%</span></div>
                    <div class="flex justify-between"><span class="text-blue-300">جيد جداً</span><span class="text-brand-200">80% - 89%</span></div>
                    <div class="flex justify-between"><span class="text-yellow-300">جيد</span><span class="text-brand-200">70% - 79%</span></div>
                    <div class="flex justify-between"><span class="text-orange-300">مقبول</span><span class="text-brand-200">60% - 69%</span></div>
                    <div class="flex justify-between"><span class="text-red-300">ضعيف</span><span class="text-brand-200">أقل من 60%</span></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-brand-100 overflow-hidden animate-fade-in">
        <div class="p-6 border-b border-brand-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <form action="{{ route('performance.index') }}" method="GET" class="flex items-center gap-2">
                ...
                <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 bg-brand-950 text-white font-bold rounded-xl hover:bg-brand-800 transition-all">
            ...
            <table class="w-full text-right">
                <thead>
                    <tr class="border-b border-brand-100 bg-white">
                        ...
                        <th class="px-6 py-4 text-xs font-bold text-brand-400 uppercase tracking-wider">النتيجة</th>
                        <th class="px-6 py-4 text-xs font-bold text-brand-400 uppercase tracking-wider">التقدير</th>
                ...
                <tbody class="divide-y divide-brand-50">
                    @forelse($evaluations as $evaluation)
                        <tr class="hover:bg-brand-50/50 transition-all duration-200">
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-1 text-xs font-bold rounded-lg bg-brand-50 text-brand-700">
                                    {{ $evaluation->period_label }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <div class="w-16 h-2 bg-brand-100 rounded-full overflow-hidden">
                                        <div class="h-full rounded-full {{ $evaluation->percentage >= 80 ? 'bg-green-500' : ($evaluation->percentage >= 60 ? 'bg-yellow-500' : 'bg-red-500') }}"
                                             style="width: {{ $evaluation->percentage }}%"></div>
                                    </div>
                                    <span class="text-sm font-bold text-brand-700">{{ $evaluation->percentage }}%</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @php $gc = $evaluation->grade_color; @endphp
                                <span class="inline-flex items-center px-2.5 py-1 text-xs font-bold rounded-lg
                                    {{ $gc === 'green' ? 'bg-green-50 text-green-700' : ($gc === 'blue' ? 'bg-blue-50 text-blue-700' : ($gc === 'yellow' ? 'bg-yellow-50 text-yellow-700' : ($gc === 'orange' ? 'bg-orange-50 text-orange-700' : 'bg-red-50 text-red-700'))) }}">
                                    {{ $evaluation->grade }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <a href="{{ route('performance.pdf', $evaluation) }}" class="p-2 text-brand-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="PDF" target="_blank">
