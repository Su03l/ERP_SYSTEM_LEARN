<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto pb-12 w-full mt-8 px-4 sm:px-6 lg:px-8" x-data="{
        ...
        <div
            ...
            {{-- Decorative Elements --}}
            <div
                class="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]"></div>
            <div
                class="absolute -top-24 -right-24 w-80 h-80 rounded-full bg-brand-500/20 blur-[100px] animate-pulse"></div>
                ...
                <div
                    ...
                    <div class="relative w-40 h-40 mx-auto mb-6 group">
                        <div
                            ...
                            <img
                                src="{{ $user->avatar ? asset('storage/' . $user->avatar) : 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&color=fff&background=020617&size=200' }}"
                    ...
                    <div class="text-center mb-8">
                        <h2 class="text-2xl font-black text-brand-950 tracking-tight capitalize">{{ $user->name }}</h2>
                        <div
                    ...
                    <div class="space-y-3 pt-6 border-t border-brand-50">
                        <button @click="tab = 'profile'" type="button"
                                :class="tab === 'profile' ? 'bg-brand-900 text-white shadow-lg shadow-brand-900/20 translate-x-1' : 'text-brand-600 hover:bg-brand-50'"
                        ...
                        <button @click="tab = 'password'" type="button"
                                :class="tab === 'password' ? 'bg-brand-900 text-white shadow-lg shadow-brand-900/20 translate-x-1' : 'text-brand-600 hover:bg-brand-50'"
                                class="w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all duration-300 font-black text-sm">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path
                        ...
                        <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div
                                    ...
                                    <div
                                        class="absolute top-0 right-0 w-24 h-24 bg-brand-900/5 rounded-full -mr-12 -mt-12"></div>
                                    <h4 class="text-sm font-black text-brand-950 mb-6 flex items-center gap-2 uppercase tracking-widest">
                            ...
                            <div class="flex justify-end pt-8">
                                <button type="submit"
                                    ...
                                    <div
                                        class="absolute inset-0 w-full h-full bg-gradient-to-r from-brand-900 to-brand-800 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        <form method="POST" action="{{ route('password.update') }}" class="max-w-xl space-y-8">
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="group">
                                    ...
                                    <x-password-input name="password" class="w-full rounded-2xl border border-brand-200 !bg-brand-50/20"/>
                                </div>
                            </div>
        ...
        <div x-show="showAvatarModal" x-transition.opacity
            ...
            <div @click.outside="showAvatarModal = false"
                ...
                <h3 class="text-xl font-black text-center text-brand-950 mb-10 tracking-tight uppercase">الصورة الشخصية</h3>

                <div
                    class="w-56 h-56 rounded-full overflow-hidden mx-auto mb-10 border-8 border-brand-50 shadow-2xl relative">
                    <img
                        src="{{ $user->avatar ? asset('storage/' . $user->avatar) : 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&color=fff&background=020617' }}"
                        class="avatar-preview w-full h-full object-cover">
                </div>
